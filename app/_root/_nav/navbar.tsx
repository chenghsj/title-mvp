...
import { checkIsMobile } from '@/lib/check-is-mobile';
...
export async function Navbar({ children }: Props) {
  ...
  return (
    <div
      className={cn(
        'sticky top-0 z-50 flex w-screen items-center justify-between border-[0.5px] border-r-0 border-b-zinc-300 bg-white text-base opacity-100 dark:border-b-zinc-600 dark:bg-zinc-950',
        'md:pr-13 px-3 py-0 md:px-10 md:py-4 xl:px-16',
        isMobile ? 'pr-5' : 'pr-8',
    ...
    >
      ...
      <div className='flex'>
        ...
        <div className='ml-4 flex items-center gap-2 md:gap-3'>
          <ThemeSwitch />
          {user ? (
            <AvatarWithDropdown profile={profile} isMobile={isMobile} />
          ) : (
            <>
              <Button
